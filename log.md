...
- Fourth sort toggle "Wenigste Umstiege / Fewest transfers" next to departure/delay/price: ascending by `journey.transfers` (bahn.de's `umstiegsAnzahl`, already in every API response; fallback = non-walking legs − 1, the same derivation the cards use), stable sort keeping departure order on ties. Generic `.sort-btn` click handler and `data-i18n` wiring picked the new button up with no further changes.
