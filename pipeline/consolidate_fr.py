...
from fr_common import consolidate_day, open_obs_db
...
def prune_old_days(days_dir: Path, cutoff: date):
    for f in sorted(days_dir.glob("*.parquet")):
        ...
        if d < cutoff:
            f.unlink()
            print(f"Pruned old FR day {d}")


def main():
    parser = argparse.ArgumentParser(description="Consolidate the FR observation store into per-day delay parquets")
    ...
    db_path = args.data_dir / "fr" / "obs.sqlite"
