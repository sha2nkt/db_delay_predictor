...
from it_common import consolidate_day, open_obs_db
...
def prune_old_days(days_dir: Path, cutoff: date):
    for f in sorted(days_dir.glob("*.parquet")):
        ...
        if d < cutoff:
            f.unlink()
            print(f"Pruned old IT day {d}")


def main():
    parser = argparse.ArgumentParser(description="Consolidate the IT observation store into per-day delay parquets")
    parser.add_argument("--data-dir", type=Path, default=PROJECT_ROOT / "data", help="base data directory")
    parser.add_argument("--stations", type=Path, default=PROJECT_ROOT / "config" / "it_scode_to_eva.json", help="RFI-code -> EVA crosswalk")
    args = parser.parse_args()

    stations = json.loads(args.stations.read_text())
    db_path = args.data_dir / "it" / "obs.sqlite"
    days_dir = args.data_dir / "it" / "days"
    ...
    conn.execute("DELETE FROM trains WHERE day < ?", (cutoff,))
    conn.commit()
    if deleted:
