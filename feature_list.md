...
| If-missed alternative connection | partial | every tight transfer in future mode carries `ifMissed` — the next realistic connection to the destination assuming arrival at the transfer + median delay (server-side `_if_missed_connection`, one cached bahn.de replan per tight transfer); shown behind a "Falls verpasst: Ankunft HH:MM" disclosure button on the warning strip, alternative legs with own rail run + delay badges, original legs from the missed departure onward crossed out while open (earliest open miss wins); implemented 2026-08-03, browser verification pending |
