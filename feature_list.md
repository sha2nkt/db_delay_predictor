...
| Ko-fi donate link | done | plain links to ko-fi.com/delaybahn (no widget/script): muted footer link + one-line "Hat dir das geholfen?" nudge under the results, shown only after a successful search, hidden on search start/mode switch/empty results; `track("donate", {placement})` Umami events; Playwright-verified incl. tracking payloads (2026-07-27) |
